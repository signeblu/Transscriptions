...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        else if (a === '--latex' || a === '--tex') args.format = 'latex';
...
Options:
  ...
  --latex, --tex       Output LaTeX transcript using scriptsize + multicols
...
function buildTranscriptParagraphs(segments, { paragraphGapSec = 1.2, maxParagraphSec = 30 } = {}) {
...
function formatTxtTranscript(segments, { paragraphGapSec = 1.2, maxParagraphSec = 30 } = {}) {
    const paragraphs = buildTranscriptParagraphs(segments, { paragraphGapSec, maxParagraphSec });
...
function formatLatexTranscript(segments, {
    paragraphGapSec = 1.2,
    maxParagraphSec = 30,
    speaker = 'SPEAKER',
    vspace = '0.3cm',
    ...
    const body = paragraphs.map((p) => {
        const text = escapeLatex(p.parts.join(' ').replace(/\s+/g, ' ').trim());
        const timestamp = `[${fmtClock(p.start)}]`;

        return `\\textbf{${escapeLatex(speaker)}:}\\\\
\\textbf{${timestamp}} ${text}
...
async function transcribeFile({
    ...
    try {
        ...
        if (!useChunking) {
            ...
            if (format === 'latex') return formatLatexTranscript(segs);
        ...
        try {
            ...
            if (format === 'latex') return formatLatexTranscript(merged);
